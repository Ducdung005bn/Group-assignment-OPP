...
import main.classes.other.opponents.EmailSender;
...
public class MainInterfaceController {
    ...
    private void downLoadDocumentsPDF(LibraryManagementSystem libraryManagementSystem) {
        ...
        downloadPdfButton.addActionListener(e -> {
            try {
                String fileLink = "LibraryDocuments.pdf";
                LibraryPdfGenerator pdfGenerator = new LibraryPdfGenerator();
                pdfGenerator.generateLibraryPdf(fileLink, libraryManagementSystem.getAllDocuments());

                // Create and send the email
                EmailSender emailSender = new EmailSender();
                String subject = "PDF of Documents' List";
                String messageText = "Link to PDF file: ";

                String email = "";
                if (isDealingWithBorrower) {
                    email = borrower.getUserEmailAddress();
                } else {
                    email = librarian.getUserEmailAddress();
                }
                emailSender.sendEmailWithAttachment(email, subject, messageText, fileLink);
    ...
    private void displayBooks(JPanel jpn, String kind) {
        ...
        // Create an ExecutorService to manage threads efficiently
        ...
        for (Document book : books) {
            ...
            executor.submit(() -> {
                try {
                    // Load the book cover image in the background
                    ImageIcon bookCover = new ImageIcon(book.getDocumentImageUrl());

                    // Add book cover to the book panel on the Event Dispatch Thread (EDT)
